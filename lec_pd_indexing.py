...
#   Outline:
#
#   1. Selection using loc.py (label based)
#     1.1 Series:
...
# 1.1.1 Series.loc.py: Selection using a single label
# ser.loc.py[label] --> scalar if label in index, error otherwise

# Set `x` below to be the price on 2020-01-10
x = ser.loc['2020-01-10']

# The following will raise a KeyError, since '3000-01-10' does exist
# ser.loc.py['3000-01-10']


# Using .loc.py to set elements
# Copy the series
ser2 = ser.copy()
#print(ser2)
...
# 1.1.2 Series.loc.py: Selection using sequence of labels
...
# 1.1.3 Series.loc.py: Selection using slices
...
# 1.2.1 Dataframe.loc.py: Selection using a single label:
...
#print(type(df.loc.py[:,'Close'])) # --> <class 'pandas.core.series.Series'>

y = df.loc['2020-01-03', :]
print(y)

#print(type(df.loc.py['2020-01-03', :])) # --> <class 'pandas.core.series.Series'>

# When omitting column labels, pandas will return a series if the row label
# exists. Otherwise it will raise an exception

# This is equivalent to df.loc.py['2020-01-03',:]
#x = df.loc.py['2020-01-03']
#print(x)

#type(df.loc.py['2020-01-03']) # --> <class 'pandas.core.series.Series'>

# This will raise an exception because the label does not exist
#df.loc.py['2020-01-01']

# 1.2.2 Dataframe.loc.py: Selection using sequence of labels
# Set x so it contains the closing prices for '2020-01-02' and '2020-01-03'
x = df.loc[['2020-01-02', '2020-01-03'], 'Close']
print(x)


# 1.2.3 Dataframe.loc.py: Selection using slices
# Using slices will return
#  - A series if the other index is a single label
#  - A data frame otherwise

# The next statement is equivalent to x = df.loc.py['2020-01-01':'2020-01-10']
...
#x = df.loc.py['2999-01-01':'2999-01-10', :]
#print(x)
...
#type(df.loc.py['2020-01-06':, :]) # --><class 'pandas.core.frame.DataFrame'>
...
# Slices will not include endpoints, otherwise, work like ser.loc.py
x = ser.iloc[0:1]
print(x)
...
# x = ser[['2020-01-02', '3000-01-03']] # --> KeyError
